...
public interface ActiveUniqueImeiEdrRepository extends JpaRepository<ActiveUniqueEdr, Long>, JpaSpecificationExecutor<ActiveUniqueEdr> {
    ...
    Page<ActiveUniqueEdr> findAllLatestUniqueImeiInLastXDays(@Param("startDate") LocalDateTime paramLocalDateTime1, @Param("endDate") LocalDateTime paramLocalDateTime2, Pageable paramPageable);

    @Query("SELECT a FROM ActiveUniqueEdr a WHERE a.createdOn BETWEEN :startDate AND :endDate")
    Page<ActiveUniqueEdr> findAllByCreatedOnBetween(@Param("startDate") LocalDateTime paramLocalDateTime1, @Param("endDate") LocalDateTime paramLocalDateTime2, Pageable paramPageable);
