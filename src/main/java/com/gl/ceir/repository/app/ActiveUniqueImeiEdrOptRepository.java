...
public interface ActiveUniqueImeiEdrOptRepository extends JpaRepository<ActiveUniqueEdrOpt,Long>, JpaSpecificationExecutor<ActiveUniqueEdrOpt> {
    ...
    public Page<ActiveUniqueEdrOpt> findAllLatestUniqueImeiInLastXDays(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, Pageable pageable);

    @Query(value = "SELECT a FROM ActiveUniqueEdrOpt a WHERE a.createdOn BETWEEN :startDate AND :endDate")
    Page<ActiveUniqueEdrOpt> findAllByCreatedOnBetween(@Param("startDate") LocalDateTime startDate,
