...
public interface ActiveForeignImeiWithDifferentMsisdnEdrOptRepository extends JpaRepository<ActiveForeignImeiWithDifferentImsiOpt, Long>, JpaSpecificationExecutor<ActiveForeignImeiWithDifferentImsiOpt> {
    ...
    Page<ActiveForeignImeiWithDifferentImsiOpt> findAllLatestDifferentImeiInLastXDays(@Param("startDate") LocalDateTime paramLocalDateTime1, @Param("endDate") LocalDateTime paramLocalDateTime2, Pageable paramPageable);
    ...
    @Query("SELECT a FROM ActiveForeignImeiWithDifferentImsiOpt a WHERE a.createdOn BETWEEN :startDate AND :endDate")
    Page<ActiveForeignImeiWithDifferentImsiOpt> findAllByCreatedOnBetween(@Param("startDate") LocalDateTime paramLocalDateTime1, @Param("endDate") LocalDateTime paramLocalDateTime2, Pageable paramPageable);
}
